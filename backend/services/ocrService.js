...
exports.extractDataWithGemini = async (filePath, mimeType, engine = 'gemini', options = {}) => {
  if (engine === 'gemini') {
    ...
    try {
      ...
      const isPartial = options && options.scanMode === 'partial';
      const prompt = isPartial ? `
    คุณเป็นผู้ช่วยผู้เชี่ยวชาญในการสกัดข้อมูลเฉพาะส่วนจากเอกสารราชการไทย (Thai Official Documents)
    เนื่องจากเอกสารฉบับนี้เป็นงานเดิมในระบบแล้ว ให้สกัดเฉพาะข้อมูลที่จำเป็นโดยเน้นความครบถ้วน 100%:
    - sign_date (วันที่ลงนาม YYYY-MM-DD จากลายเซ็นท้ายเอกสาร)
    - meeting_date (วันที่ประชุม YYYY-MM-DD)
    - reply_due_date (วันกำหนดส่งตอบรับ YYYY-MM-DD)
    - notes (หมายเหตุ)
    - main_text: ข้อความเนื้อหาทั้งหมดที่อยู่หลังคำว่า 'เรียน' มาจนถึงก่อนส่วนสั่งการท้ายเอกสาร **ห้ามตัดทอนข้อความ ห้ามสรุปความเด็ดขาด ให้คัดลอกเนื้อหาต้นฉบับภาษาไทยทั้งหมด 100% ตามเอกสารจริง** (หากมีหลายย่อหน้าให้ขึ้นบรรทัดใหม่ด้วย \n)
    - task_detail: รายละเอียดสิ่งที่ต้องดำเนินการ/การมอบหมายงาน ให้คัดลอกข้อความย่อหน้าขีด (-) ทั้งหมดที่อยู่ใต้ชื่อยศ/ตำแหน่งการมอบหมายงานสั่งการท้ายเอกสาร (เช่น ใต้ยศ ผบก.รน., รอง ผบ.ตร.) **โดยต้องคัดลอกข้อความย่อหน้าขีด (-) มาแบบ 100% ตามที่ปรากฏจริงในเอกสารเท่านั้น ห้ามเรียบเรียงใหม่ ห้ามแต่งประโยคใหม่เด็ดขาด** (ตัวอย่าง: "- เป็นผู้แทน ตร. เข้าร่วมประชุมฯ ตามวันและเวลาดังกล่าว\n- รายงานผลการประชุมฯ")

    ให้สกัดข้อมูลออกมาเป็นรูปแบบ JSON อย่างเคร่งครัด:
    {
      "full_text": "ข้อความดั้งเดิมทั้งหมด",
      "memos": [
        {
          "sign_date": "YYYY-MM-DD หรือ null",
          "meeting_date": "YYYY-MM-DD หรือ null",
          "reply_due_date": "YYYY-MM-DD หรือ null",
          "notes": "ข้อความหมายเหตุ หรือ null",
          "main_text": "เนื้อหาหลักสกัดครบถ้วน 100% ทุกย่อหน้า (ห้ามสรุปความ ห้ามตัดทอนข้อความ)",
          "task_detail": "ข้อความย่อหน้าขีด (-) ท้ายเอกสาร 100%"
        }
      ]
    }

    **กฎเกณฑ์สำคัญในการสกัดข้อมูล:**
    1. **main_text**: ทุกอย่างที่อยู่ **หลังเส้น "เรียน"** มาจนถึงก่อนส่วนสั่งการท้ายเอกสาร จะต้องสกัดมาให้ครบทั้งหมด (ห้ามสรุปความ ห้ามตัดคำทิ้ง ให้ดึงมาทั้งหมด 100%)
    2. **task_detail**: คัดลอกเฉพาะข้อความย่อหน้าขีด (-) จากส่วนสั่งการท้ายเอกสารมาแบบ 100% ตามที่ปรากฏจริงในเอกสารเท่านั้น
    3. **สำคัญมาก: ห้ามใช้ Emoji หรือ Emoticon ใดๆ ในผลลัพธ์โดยเด็ดขาด ให้ใช้เฉพาะข้อความ Text ปกติเท่านั้น**

    คืนค่าเป็น JSON string เท่านั้น ห้ามใส่ข้อความอธิบาย ห้ามใช้ Markdown block (\`\`\`json)
      ` : `
      ...
      "memos": [
        {
          ...
          "main_text": "ข้อความเนื้อหาทั้งหมดที่อยู่หลังคำว่า 'เรียน' (ยกเว้นส่วนการมอบหมายงาน) **ห้ามตัดทอนข้อความ ห้ามสรุปความเด็ดขาด ให้ดึงเนื้อหาต้นฉบับมาแบบ 100% ทุกย่อหน้า** หน้าที่ของคุณคือเพียงแค่เพิ่มการจัดย่อหน้า (ขึ้นบรรทัดใหม่ด้วย \\n) และทำตัวหนา (ใส่ **เน้นคำ**) ในจุดที่สำคัญ เพื่อให้อ่านง่ายขึ้นเท่านั้น",
          "task_detail": "รายละเอียดสิ่งที่ต้องดำเนินการ/การมอบหมายงาน ให้คัดลอกข้อความย่อหน้าขีด (-) ทั้งหมดที่อยู่ใต้ชื่อยศ/ตำแหน่งการมอบหมายงานสั่งการท้ายเอกสาร (เช่น ใต้ยศ ผบก.รน., รอง ผบ.ตร.) **โดยต้องคัดลอกข้อความย่อหน้าขีด (-) มาแบบ 100% ตามที่ปรากฏจริงในเอกสารเท่านั้น ห้ามเรียบเรียงใหม่ ห้ามแต่งประโยคใหม่เด็ดขาด** (ตัวอย่าง: '- เป็นผู้แทน ตร. เข้าร่วมประชุมฯ ตามวันและเวลาดังกล่าว\\n- รายงานผลการประชุมฯ')",
          "assignments": [
            {
    ...
    **กฎเกณฑ์สำคัญในการสกัดข้อมูล:**
    ...
    2. **main_text**: ทุกอย่างที่อยู่ **หลังเส้น "เรียน"** จะต้องอยู่ในหมวดนี้ (ห้ามสรุปความ ห้ามตัดคำทิ้ง ให้ดึงมาทั้งหมด 100%)
    3. **task_detail**: คัดลอกเฉพาะข้อความย่อหน้าขีด (-) จากส่วนสั่งการท้ายเอกสารมาแบบ 100% ตามที่ปรากฏจริงในเอกสารเท่านั้น
